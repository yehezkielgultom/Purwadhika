...
# hitung_keuntungan(): Fungsi ini menampilkan laporan keuntungan yang diperoleh dari barang yang telah di-checkout oleh pembeli. Jika belum ada barang yang di-checkout, pesan akan ditampilkan. 
# Laporan mencakup total harga beli (modal), total uang masuk (kotor), total diskon diberikan, total uang masuk (kotor) setelah diskon, total keuntungan (bersih) 
def hitung_keuntungan():
    ...
    print("\n=== Laporan Penjualan ===")
    print(f"{'Nama Barang':<20} {'Jumlah':<9} {'Harga Jual Satuan':<20} {'Harga Jual Total':<20} {'Diskon':<15} {'Keuntungan':<15}")
    print("=" * 100)
    ...
    for item in barang_terjual:
        jumlah = item['Jumlah']
        harga_jual_satuan = item['Harga Jual']
        total_harga_jual = harga_jual_satuan * jumlah
        diskon = 0

        # Hitung diskon jika jumlah barang lebih dari 12
        if jumlah > 12:
            diskon = total_harga_jual * 0.1  # 10% diskon
            total_harga_jual -= diskon

        # Hitung total modal (harga beli * jumlah)
        modal = (item['Harga Jual'] / 1.2) * jumlah  # Mengambil harga beli dari harga jual

        # Hitung keuntungan setelah diskon
        keuntungan = total_harga_jual - modal

        # Tampilkan hasil setiap barang
        print(f"{item['Nama']:<20} {jumlah:<9} Rp{int(harga_jual_satuan):<18} Rp{int(total_harga_jual):<18} Rp{int(diskon):<12} Rp{int(keuntungan):<15}")

        total_uang_masuk += total_harga_jual
        total_keuntungan += keuntungan
        total_modal += modal
        total_diskon_diberikan += diskon

    print("=" * 100)
    print(f"{'Total Harga Beli (Modal):':<42} Rp{int(total_modal):<15}")
    print(f"{'Total Uang Masuk (Kotor) Sebelum Diskon:':<42} Rp{int(total_uang_masuk + total_diskon_diberikan):<15}")
    print(f"{'Total Diskon Diberikan:':<42} Rp{int(total_diskon_diberikan):<15}")
    print(f"{'Total Uang Masuk (Kotor) Setelah Diskon:':<42} Rp{int(total_uang_masuk):<15}")
    print(f"{'Total Keuntungan (Bersih):':<42} Rp{int(total_keuntungan):<15}")
